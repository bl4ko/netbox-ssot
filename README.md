...
| `source.vlanGroupRelations`              | Regex relations in format `regex = vlanGroup`, that map each vlan that satisfies regex to vlanGroup. For **ovirt**, when no relation matches, the datacenter name is used as the default VLAN Group (ensures correct scoping across multiple datacenters). | all                        | []string | any                                      | []         | No       |
...
| `source.domainName`                      | Domain name for user authentication. If prefixed with `id|`, it acts as `domainID` directly. Ignored if `domainID` is set. | [**openstack**]            | string   | any                                      | "Default"  | No       |
| `source.domainID`                        | ID of the OpenStack domain for user authentication. Takes precedence over `domainName` when both are provided.          | [**openstack**]            | string   | any                                      | ""         | No       |
| `source.projectDomainName`               | Domain name for project scoping. Defaults to `domainName` if not specified. Ignored if `projectDomainID` is set.        | [**openstack**]            | string   | any                                      | ""         | No       |
| `source.projectDomainID`                 | ID of the project domain for project scoping. Defaults to `domainID` if not specified. Takes precedence over `projectDomainName` when both are provided. | [**openstack**]            | string   | any                                      | ""         | No       |
