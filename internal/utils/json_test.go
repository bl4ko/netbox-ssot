...
func TestExtractJSONTagsFromStruct(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: []string{"id", "name", "label", "type", "object_types", "description", "search_weight", "filter_logic", "ui_visible", "ui_editable", "weight", "default", "required"},
		},
		{
			...
			want: []string{"id", "tags", "description", "custom_fields", "name", "role", "device_type", "airflow", "serial", "asset_tag", "site", "location", "status", "platform", "primary_ip4", "primary_ip6", "cluster", "tenant", "comments"},
		},
		{
			...
			want: []string{"id", "tags", "description", "custom_fields", "virtual_machine", "name", "primary_mac_address", "mtu", "enabled", "parent", "bridge", "mode", "tagged_vlans", "untagged_vlan"},
		},
		{
			...
			want: []string{"id", "tags", "description", "custom_fields", "device", "name", "enabled", "type", "speed", "parent", "bridge", "lag", "mtu", "primary_mac_address", "duplex", "mode", "tagged_vlans", "untagged_vlan", "vdcs"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ExtractJSONTagsFromStruct(tt.args.inputStruct); !reflect.DeepEqual(got, tt.want) {
