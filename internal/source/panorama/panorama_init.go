...
func (pas *PaloAltoSource) initSystemInfo(c *pango.Panorama) error {
	pas.SystemInfo = c.Client.SystemInfo
	return nil
}

func (pas *PaloAltoSource) initVirtualSystems(c *pango.Panorama, tmpl string, ts string) error {
	virtualSystems, err := c.Vsys.GetAll(tmpl, ts)
	if err != nil {
	...
	for _, virtualSystem := range virtualSystems {
		securityZones, err := c.Network.Zone.GetAll(tmpl, ts, virtualSystem.Name)
		if err != nil {
...
func (pas *PaloAltoSource) initVirtualRouters(c *pango.Panorama, tmpl string, ts string) error {
	routers, err := c.Network.VirtualRouter.GetAll(tmpl, ts)
	if err != nil {
		return err
	}
...
func (pas *PaloAltoSource) initInterfaces(c *pango.Panorama, tmpl string, ts string) error {
	ethInterfaces, err := c.Network.EthernetInterface.GetAll(tmpl, ts)
	if err != nil {
		return err
	}
	...
	for _, ethInterface := range ethInterfaces {
		...
		subInterfaces, err := c.Network.Layer3Subinterface.GetAll(tmpl, ts, layer3.EthernetInterface, ethInterface.Name)
		if err != nil {
...
func (pas *PaloAltoSource) initArpData(c *pango.Panorama) error {
