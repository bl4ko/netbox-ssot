...
func (hcs *Source) syncServers(nbi *inventory.NetboxInventory) error {
	...
	if err != nil {
		return fmt.Errorf("creating cluster type: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("creating cluster: %s", err)
	}
	...
	for _, server := range hcs.Servers {
		if err := hcs.syncServer(nbi, server, netboxCluster, netboxRole); err != nil {
			return err
		}
	}
...
func (hcs *Source) syncServer(
	...
	vm := &objects.VM{
		...
		Memory:   gbToMB(server.ServerType.Memory),
		Disk:     server.ServerType.Disk * mbPerGB,
	}
	...
	if server.PublicNet.IPv6.IP != nil && server.PublicNet.IPv6.Network != nil {
		...
		// Append "1" to represent the host IP inside the /64 block.
	...
	for i, privateNet := range server.PrivateNet {
		...
		if privateNet.MACAddress != "" {
			...
			if err = common.SetPrimaryMACForInterface(hcs.Ctx, nbi, netboxPrivIface, nbMAC); err != nil {
...
func (hcs *Source) syncNetworks(nbi *inventory.NetboxInventory) error {
	for _, network := range hcs.Networks {
		...
		if err != nil {
			hcs.Logger.Errorf(hcs.Ctx, "syncing network %s: %s", network.Name, err)
		}
	}
	return nil
}

func (hcs *Source) syncFloatingIPs(nbi *inventory.NetboxInventory) error {
	for _, fip := range hcs.FloatingIPs {
		ipAddr := &objects.IPAddress{
			...
			Address: fmt.Sprintf("%s/32", fip.IP.String()),
			Status:  &objects.IPAddressStatusActive,
		}

		_, err := nbi.AddIPAddress(hcs.Ctx, ipAddr)
		if err != nil {
			hcs.Logger.Errorf(hcs.Ctx, "syncing floating IP %s: %s", fip.IP.String(), err)
		}
	}
	return nil
}
