...
func (vc *VmwareSource) syncClusters(nbi *inventory.NetBoxInventory) error {
	...
	// for _, cluster := range vc.Clusters {
	...
	// 	if !exists {
	// 		vc.Logger.Warning("description for oVirt cluster ", clusterName, " is empty.")
	// 	}
	// 	var clusterGroup *objects.ClusterGroup
	// 	if _, ok := vc.DataCenters[cluster.MustDataCenter().MustId()]; ok {

	// 	} else {
	// 		vc.Logger.Warning("failed to get datacenter for oVirt cluster ", clusterName)
	// 	}
...
func (vc *VmwareSource) syncHosts(nbi *inventory.NetBoxInventory) error {
	...
	// 	if !exists {
	// 		vc.Logger.Warning("name for oVirt host with id ", hostId, " is empty.")
	// 	}
	// 	hostCluster := nbi.ClustersIndexByName[vc.Clusters[host.MustCluster().MustId()].MustName()]
	...
	// 		if !exists {
	// 			vc.Logger.Warning("Uuid (asset tag) for oVirt host ", hostName, " is empty. Can't identify it, so it will be skipped...")
	// 			continue
	// 		}
	// 		hostSerialNumber, exists = hwInfo.SerialNumber()
	// 		if !exists {
	// 			vc.Logger.Warning("Serial number for oVirt host ", hostName, " is empty.")
	// 		}
	...
	// 		}
	// 	} else {
	// 		vc.Logger.Warning("Hardware information for oVirt host ", hostName, " is empty, it can't be identified so it will be skipped.")
	// 		continue
	// 	}
	...
	// 		if !exists {
	// 			vc.Logger.Warning("oVirt hostCpuCores of ", hostName, " is empty.")
	// 		}
	// 	}
	...
	// 		NetboxObject: objects.NetboxObject{Description: hostDescription, Tags: vc.SourceTags},
	// 		Name:         hostName,
	// 		Status:       hostStatus,
...
func (vc *VmwareSource) syncHostNics(nbi *inventory.NetBoxInventory, ovirtHost *ovirtsdk4.Host, nbHost *objects.Device) error {
